  ...
  return (
    <section className="p-6 bg-gray-900 rounded-lg shadow-lg border border-gray-800">
      <div className="flex justify-between items-center mb-6 border-b border-gray-800 pb-2">
        ...
        <button onClick={handleAdd} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-md transition-colors">+ Add Link</button>
      </div>
      ...
      <div className="space-y-4">
        {portfolio.map((item) => (
          <div key={item.id} className="grid grid-cols-1 md:grid-cols-[1fr_2fr_auto] gap-4 items-end bg-gray-800 p-4 border border-gray-700 rounded-md">
            ...
            <button onClick={() => updateList('portfolio', portfolio.filter(p => p.id !== item.id))} className="mb-4 px-3 py-2 text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded transition-colors">Remove</button>
          </div>
        ))}
      </div>
    </section>
